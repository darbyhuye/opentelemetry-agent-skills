...
| `tail_sampling` | `components/tail_sampling/README.md` | processor | traces | Beta | Buffers whole traces and makes a single keep/drop decision after a wait window via policies. v0.159.0 added in-process `num_shards`; scaling across Collector instances still requires a `load_balancing` layer. |
...
| `resource_detection` | `components/resource_detection/README.md` | processor | traces, metrics, logs, profiles | Beta (traces/metrics/logs), Development (profiles) | Auto-detects resource attributes via an ordered `detectors` list. v0.159.0 added global retry settings and the `azurecontainerapps` detector; v0.158.0 added top-level `fail_on_missing_metadata`. Renamed from `resourcedetection` in v0.153.0; alias preserved. |
