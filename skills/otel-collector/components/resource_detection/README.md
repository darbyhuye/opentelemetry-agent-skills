...
v0.159.0 added a global `retry` block for detector failures and the `azurecontainerapps` detector.
Retries are enabled by default, with the overall default session bounded by the 5s HTTP timeout.
